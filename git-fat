...
def setup_logging():
    log_level = logging.DEBUG if os.environ.get("GIT_FAT_VERBOSE") else logging.INFO
...
class GitFat(object):
    ...
    def __init__(self):
        try:
            ...
            sys.exit(1)
        self.gitdir = (
            subprocess.check_output("git rev-parse --git-dir".split())
            .strip()
            .decode("utf-8")
        )
        ...
        self.magiclens = [
            magiclen(enc) for enc in [self.encode_v1, self.encode_v2]
        ]  # All prior versions
